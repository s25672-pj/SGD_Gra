...
bool Game::initialize() {
    ...
    spikes->lockInPlace();
    spikes -> getCollider()->isPhysicsObject = true;
    spikes -> getCollider()->isStaticObject = true;

    auto imperator = new TestObject("Impretaor" , -200, 200, ResourceManager::loadTexture("imperator.png"),29,29+40);
    imperator->getCollider()->isPhysicsObject = true;

    player = new Player("Player", -100, 200, ResourceManager::loadTexture("anim.png"), 8, 29, 12, 3);
    player -> getCollider()->isPhysicsObject = true;
    Environment::init();
    ...
    if (!pause1) {
        ...
        if (keystates[SDL_SCANCODE_H]){
            if (!enemy) {
                ...
                enemy->getCollider()->isStaticObject = true;
            }
        }
        ...
        for (int i = 0; i < TestObject::registry.size(); ++i) {
            auto obj = TestObject::registry[i];
            obj->isGrounded = false;
            obj->test = false;
            CollisionManager::checkCollisions(obj, deltaTime);
...
void Game::loop() {
    ...
    while (running) {
        ...
        std::chrono::duration <double> duration = currentTime - previousTime;

        double deltaTime = duration.count();
        Globals::deltaTime = deltaTime;
        previousTime = currentTime;
        Globals::curTime += deltaTime;
